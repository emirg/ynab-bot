...
class TestUserNow:
    ...
    def test_user_now_utc_minus_5_difference(self):
        """Bogota (UTC-5) and UTC differ by exactly 5 hours."""
        utc_now = user_now("UTC")
        bog_now = user_now(DEFAULT_TIMEZONE)
        # Convert both to UTC for comparison
        utc_ts = utc_now.astimezone(ZoneInfo("UTC"))
        bog_ts = bog_now.astimezone(ZoneInfo("UTC"))
        # They should be within a few seconds of each other (same instant)
        diff = abs((utc_ts - bog_ts).total_seconds())
        assert diff < 2  # less than 2 seconds apart
        # But their naive hours should differ by 5 (Bogota has no DST)
        offset_hours = bog_now.utcoffset().total_seconds() / 3600

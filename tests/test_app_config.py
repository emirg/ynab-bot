...
class TestFromEnv:
    ...
    def test_defaults_to_postgres_persistence_backend(self, monkeypatch, tmp_path):
        self._clear_env(monkeypatch)
        env_file = tmp_path / '.env'
        env_file.write_text(
            'APP_MODE=http-dev\n'
    ...
    def test_sqlite_backend_is_rejected(self, monkeypatch, tmp_path):
        ...
        with pytest.raises(ConfigurationException, match='Invalid PERSISTENCE_BACKEND'):
            AppConfig.from_env(str(env_file))

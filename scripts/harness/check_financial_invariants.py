...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run financial invariant harness diagnostics.")
    parser.add_argument("--root", default=".", help="Repository root to check.")
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Exit nonzero when financial invariant failures are found.",
    )
    parser.add_argument("--json", action="store_true", help="Print machine-readable JSON diagnostics.")
    args = parser.parse_args(argv)

    findings = check_financial_invariants(Path(args.root).resolve())
